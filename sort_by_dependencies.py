#!/usr/bin/env python3
"""
Sort Hanzi Deck by Radical Dependencies
========================================
Organizes the deck into levels where:
- Level 1-57: Radicals (5 radicals per level, 285 total)
...
def assign_radical_levels(radicals_df):
    ...
    5 radicals per level (Level 1-57 for 285 radicals)
    """
...
def calculate_hanzi_level(radicals_list, radical_to_level, radical_levels):
    ...
    The hanzi level is the maximum level of any of its radicals.
    ...
    for radical in radicals_list:
        ...
        max_level = max(max_level, radical_level)
...
def assign_hanzi_levels(hanzi_df, radical_to_level, radical_levels):
    ...
    Each hanzi's level is determined by the highest-level radical it contains.
    """
    ...
    # Sort by level, then by frequency rank (if available)
    if 'frequency_rank' in hanzi_df.columns:
        hanzi_df = hanzi_df.sort_values(['level', 'frequency_rank'])
    else:
...
def calculate_vocab_level(word, characters_list, hanzi_to_level, max_hanzi_level):
    ...
    The vocab level is the maximum level of any of its characters.
    """
    ...
    for char in characters_list:
        ...
        max_level = max(max_level, char_level)
...
def assign_vocab_levels(vocab_df, hanzi_to_level, max_hanzi_level):
    ...
    Each word's level is determined by the highest-level hanzi it contains.
    """
...
def print_level_summary(radicals_df, hanzi_df, vocab_df):
    ...
    for level in sorted(radical_levels.index[:10]):  # Show first 10 levels
        ...
        print(f"   Level {level:2d}: {count} radicals")
...
def main():
    print("="*60)
    print("🎴 TIAN HANZI DECK - DEPENDENCY-BASED SORTING")
    print("="*60)
    print("\nThis script organizes the deck so that:")
    ...
    try:
        ...
        print("\n✅ Sorting complete! Data is now organized by dependency levels.")
        print("\n🎯 Next step: python create_deck_from_parquet.py")
